...
| D-017 | 2026-08-15 | Verificação de procedência compara o **objeto normalizado**, não as chaves cruas do arquivo | Os dois corpora são transcrições deliberadamente parciais. Comparar a união das chaves faz o veredicto ser sempre "diverge", e uma verificação que estruturalmente não pode passar não fecha lacuna — produz a aparência de rigor. O que importa é se os dois lados normalizam para a mesma coisa que o sistema consome | um campo não transcrito passar a afetar um número derivado |
| D-018 | 2026-08-15 | `--write` grava a procedência no corpus; sem isso a verificação não fecha lacuna | `G-008` e `G-108` falam do **corpus**, não de um comando. Um `verify` que imprime e esquece deixa a lacuna aberta na prática, por mais verde que seja a saída | — |
| D-019 | 2026-08-15 | Procedência conferida **altera o status** dos derivados, não só um campo de metadado | Se conferir a fonte não muda nada a jusante, a conferência é cerimônia. Com a ficha conferida, `parametros` sai como `OBSERVATION` e não `ASSUMPTION`, e a dívida de evidência do silício caiu de 38,6 % para 35,0 % — o efeito é medido, não alegado | — |
