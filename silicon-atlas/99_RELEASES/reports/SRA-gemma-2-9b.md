...
# Silicon Readiness Assessment — gemma-2-9b
...
Quais subgrafos de inferencia de **gemma-2-9b** sao simultaneamente estaveis, dominantes em custo e tolerantes a baixa precisao o suficiente para justificar especializacao em hardware — e a partir de qual volume isso se paga?

## 2. Afirmacao principal

Sob as condicoes declaradas, a regiao endurecivel de **gemma-2-9b** cobre **0.0%** do custo de servico, o que limita o ganho de sistema a **1.00x** numa arquitetura hibrida. Nao existe break-even a reportar: regiao fixa vazia (nenhum FLOP endurecido e nenhum peso a fixar): nao ha ponto de projeto a simular, de modo que nenhum acelerador foi dimensionado para 1e+13 tokens/ano.
...
1. **99% do custo** esta em operadores regulares e quantizaveis, barrados *apenas pela estabilidade*. Acao util: prototipar em FPGA para medir o ganho real antes de qualquer compromisso de mascara.
...
|---|---|---|
| tokens/s | 27,916 | 138.7 |
| intensidade aritmetica (FLOP/byte) | 547.6 | 1.04 |
| ponto de inflexao do dispositivo | 253 | 92 |
| tempo limitado por memoria | 18.4% | 99.9% |
| energia por token | 40.62 mJ | 8173.4 mJ |

O decode consome 98.1% do tempo da requisicao. Toda decisao de
...
**50.0%** (18.48 GB para
9.24 GB por passo de decode), com aceleracao estimada de
1.92x. O **footprint residente** e grandeza distinta:
18.48 GB, contra
80 GB de capacidade
(cabe,
folga +76.9%).
...
**SRS = 0.168** (OPEN_GAP) -> banda **software**

| Fator | Valor | Peso | Contribuicao | Status |
|---|---|---|---|---|
| A — arch_stability | 0.000 | +0.15 | +0.0000 | `OPEN_GAP` |
| H — concentration | 0.171 | +0.15 | +0.0256 | `CONDITIONAL_RESULT` |
| T — throughput | 0.667 | +0.15 | +0.1000 | `ASSUMPTION` |
| P — perf_per_watt | 0.000 | +0.15 | +0.0000 | `CONDITIONAL_RESULT` |
| Q — low_precision | 0.674 | +0.10 | +0.0674 | `ASSUMPTION` |
| D — data_availability | 0.250 | +0.10 | +0.0250 | `ASSUMPTION` |
...
top-1 estavel em **87.6%** das amostras, top-3 em
99.2%, banda de decisao do SRS estavel em
100.0%. Fator dominante: `R`.

Dispersao dos scores: 0.1354. Largura do ruido induzido pelos pesos:
0.0267. **Diagnostico: score NAO discrimina: entre os 6 candidatos que disputam a primeira posicao, 70% do peso esta em fatores identicos; a margem (0.0044) e menor que o ruido (0.0267).**
...
```json
{
  ...
  "distinct_layer_patterns": 3
}
```

## Anexo B — Estabilidade da familia `gemma`

Versoes comparadas: gemma-2-9b
...
- Modelo: `gemma-2-9b`, familia `gemma`, licenca `gemma`
- Fonte da configuracao: https://huggingface.co/google/gemma-2-9b/resolve/main/config.json
- Status de proveniencia: `TRANSCRIBED_UNVERIFIED` (lacuna G-008 aberta)
- Parametros derivados da IR: 9,241,705,984 | publicados: 9,241,705,984
...
|---|---|---|---|
| `gemma-2-9b.params` | 9241705984 params | `ASSUMPTION` | G-001, G-008 |
| `gemma-2-9b.serving.decode_time_share` | 0.9805 fracao do tempo de requisicao | `CONDITIONAL_RESULT` | G-003, G-009 |
| `gemma-2-9b.serving.energy_per_token` | 8.336 J/token gerado | `CONDITIONAL_RESULT` | G-003, G-004, G-009 |
| `gemma-2-9b.decode.tokens_per_s` | 138.7 tok/s | `CONDITIONAL_RESULT` | G-003 |
| `gemma-2-9b.decode.memory_bound_share` | 0.9991 fracao do tempo | `CONDITIONAL_RESULT` | G-003 |
| `gemma-2-9b.decode.arith_intensity` | 1.041 FLOP/byte | `CONDITIONAL_RESULT` | G-003 |
| `gemma-2-9b.decode.energy_per_token` | 8.173 J/token | `CONDITIONAL_RESULT` | G-003, G-004 |
| `quant.sensitivity.memory_reduction` | 0.5 fracao | `CONDITIONAL_RESULT` | G-002, G-003 |
| `quant.sensitivity.speedup` | 1.917 x sobre baseline | `CONDITIONAL_RESULT` | G-002, G-003 |
| `quant.sensitivity.quality_delta` | — perplexidade / acuracia | `OPEN_GAP` | G-002 |
| `quant.reachable_cost_share.int4` | 0.7344 fracao do tempo | `CONDITIONAL_RESULT` | G-002, G-003 |
| `gemma-2-9b.partition.hardened_share` | 0 fracao do tempo | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
| `gemma-2-9b.partition.amdahl_ceiling` | 1 x sobre baseline | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
...
| `SRS.gemma-2-9b` | 0.168 score SRS | `OPEN_GAP` | G-001, G-002, G-003, G-006, G-008, G-009 |
| `readiness.rank_stability` | 0.876 fracao das amostras com mesmo top-1 | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
| `stability.gemma.exact` | — fracao de blocos preservados | `OPEN_GAP` | G-006 |
