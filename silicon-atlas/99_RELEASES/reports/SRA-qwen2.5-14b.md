...
model: qwen2.5-14b
run_id: 20260820T015323Z
generated_at: 2026-08-20T01:53:23+00:00
method: DOUVRAS 2.0
cycle: C-001
...
# Silicon Readiness Assessment — qwen2.5-14b
...
Quais subgrafos de inferencia de **qwen2.5-14b** sao simultaneamente estaveis, dominantes em custo e tolerantes a baixa precisao o suficiente para justificar especializacao em hardware — e a partir de qual volume isso se paga?

## 2. Afirmacao principal

Sob as condicoes declaradas, a regiao endurecivel de **qwen2.5-14b** cobre **0.0%** do custo de servico, o que limita o ganho de sistema a **1.00x** numa arquitetura hibrida. Nao existe break-even a reportar: regiao fixa vazia (nenhum FLOP endurecido e nenhum peso a fixar): nao ha ponto de projeto a simular, de modo que nenhum acelerador foi dimensionado para 1e+13 tokens/ano.
...
- **F2** — o padrao mais custoso muda de identidade entre versoes  
  observado: bloco mais custoso 'mlp' (papel gate_proj, 23.8% do custo): E = 0.08  
  **DISPARADO**
- **F3** — top-1 do ranking troca sob perturbacao de +-20% dos pesos, ou vence por margem menor que o ruido (criterio reforcado apos CE-001)  
  observado: estabilidade do top-1 = 0.790 (limite 0.95); margem = 0.0030 contra ruido 0.0268  
  **DISPARADO**
...
|---|---|---|
| tokens/s | 18,823 | 94.1 |
| intensidade aritmetica (FLOP/byte) | 691.3 | 1.06 |
| ponto de inflexao do dispositivo | 253 | 92 |
| tempo limitado por memoria | 13.0% | 99.9% |
| energia por token | 60.25 mJ | 12053.0 mJ |
...
|---|---|---|---|---|---|
| gate_proj | mlp | 23.8% | 48 | int4 | 0.515 |
| up_proj | mlp | 23.8% | 48 | int4 | 0.515 |
| q_proj | attention | 8.8% | 48 | int8 | 0.512 |
| v_proj | attention | 1.8% | 48 | int8 | 0.501 |
| down_proj | mlp | 23.8% | 48 | int4 | 0.500 |
| k_proj | attention | 1.8% | 48 | int8 | 0.494 |
| o_proj | attention | 8.8% | 48 | int4 | 0.475 |
| lm_head | head | 5.4% | 1 | int8 | 0.398 |

### Particao recomendada

```text
FPGA/eFPGA: 98.0% do custo
  ├── gate_proj             23.8%  regular e quantizavel, porem LHS=0.51 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── up_proj               23.8%  regular e quantizavel, porem LHS=0.51 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── down_proj             23.8%  regular e quantizavel, porem LHS=0.50 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── q_proj                 8.8%  regular e quantizavel, porem LHS=0.51 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── o_proj                 8.8%  regular e quantizavel, porem LHS=0.47 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── lm_head                5.4%  regular e quantizavel, porem LHS=0.40 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  ├── v_proj                 1.8%  regular e quantizavel, porem LHS=0.50 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
  └── k_proj                 1.8%  regular e quantizavel, porem LHS=0.49 < 0.55; E=0.08 < 0.6: prototipar antes de fixar
CPU/GPU: 1.6% do custo
  └── kv_read                1.6%  LHS=0.37 < 0.55; E=0.08 < 0.6; Q=0.00 < 0.6
```
...
**50.0%** (27.98 GB para
13.99 GB por passo de decode), com aceleracao estimada de
1.97x. O **footprint residente** e grandeza distinta:
29.54 GB, contra
80 GB de capacidade
(cabe,
folga +63.1%).
...
top-1 estavel em **79.0%** das amostras, top-3 em
100.0%, banda de decisao do SRS estavel em
97.1%. Fator dominante: `R`.

Dispersao dos scores: 0.1414. Largura do ruido induzido pelos pesos:
0.0268. **Diagnostico: score NAO discrimina: entre os 6 candidatos que disputam a primeira posicao, 70% do peso esta em fatores identicos; a margem (0.0030) e menor que o ruido (0.0268).**
...
- Modelo: `qwen2.5-14b`, familia `qwen`, licenca `apache-2.0`
- Fonte da configuracao: https://huggingface.co/Qwen/Qwen2.5-14B/resolve/main/config.json
- Status de proveniencia: `FETCHED`
- Parametros derivados da IR: 14,770,033,664 | publicados: 14,770,033,664
...
|---|---|---|---|
| `qwen2.5-14b.params` | 14770033664 params | `MODEL` | G-001 |
| `qwen2.5-14b.serving.decode_time_share` | 0.9804 fracao do tempo de requisicao | `CONDITIONAL_RESULT` | G-003, G-009 |
| `qwen2.5-14b.serving.energy_per_token` | 12.29 J/token gerado | `CONDITIONAL_RESULT` | G-003, G-004, G-009 |
| `qwen2.5-14b.decode.tokens_per_s` | 94.08 tok/s | `CONDITIONAL_RESULT` | G-003 |
| `qwen2.5-14b.decode.memory_bound_share` | 0.9991 fracao do tempo | `CONDITIONAL_RESULT` | G-003 |
| `qwen2.5-14b.decode.arith_intensity` | 1.064 FLOP/byte | `CONDITIONAL_RESULT` | G-003 |
| `qwen2.5-14b.decode.energy_per_token` | 12.05 J/token | `CONDITIONAL_RESULT` | G-003, G-004 |
| `quant.sensitivity.memory_reduction` | 0.5 fracao | `CONDITIONAL_RESULT` | G-002, G-003 |
| `quant.sensitivity.speedup` | 1.965 x sobre baseline | `CONDITIONAL_RESULT` | G-002, G-003 |
| `quant.sensitivity.quality_delta` | — perplexidade / acuracia | `OPEN_GAP` | G-002 |
| `quant.reachable_cost_share.int4` | 0.8042 fracao do tempo | `CONDITIONAL_RESULT` | G-002, G-003 |
| `qwen2.5-14b.partition.hardened_share` | 0 fracao do tempo | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
| `qwen2.5-14b.partition.amdahl_ceiling` | 1 x sobre baseline | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
...
| `SRS.qwen2.5-14b` | 0.2474 score SRS | `OPEN_GAP` | G-001, G-002, G-003, G-006, G-009 |
| `readiness.rank_stability` | 0.7905 fracao das amostras com mesmo top-1 | `OPEN_GAP` | G-001, G-002, G-003, G-006 |
