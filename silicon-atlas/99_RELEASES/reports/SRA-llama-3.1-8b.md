...
# Silicon Readiness Assessment — llama-3.1-8b
...
Quais subgrafos de inferencia de **llama-3.1-8b** sao simultaneamente estaveis, dominantes em custo e tolerantes a baixa precisao o suficiente para justificar especializacao em hardware — e a partir de qual volume isso se paga?

## 2. Afirmacao principal

Sob as condicoes declaradas, a regiao endurecivel de **llama-3.1-8b** cobre **0.0%** do custo de servico, o que limita o ganho de sistema a **1.00x** numa arquitetura hibrida. Nao existe break-even a reportar: regiao fixa vazia (nenhum FLOP endurecido e nenhum peso a fixar): nao ha ponto de projeto a simular, de modo que nenhum acelerador foi dimensionado para 1e+13 tokens/ano.
...
Isto **e** o resultado: para llama-3.1-8b, o bloco que domina o custo (mlp, 24.5% do tempo) tem estabilidade estrutural E = 0.46, abaixo do limite da politica. O gasto em mascara nao tem o que financiar. A decisao economica so passa a existir se essa estabilidade subir — por escopo declarado mais estreito, por observacao de mais versoes, ou por calibracao da politica contra casos reais (`G-011`).
...
- Modelo: `llama-3.1-8b`, familia `llama`, licenca `llama3.1`
- Fonte da configuracao: https://huggingface.co/meta-llama/Llama-3.1-8B/resolve/main/config.json
...
|---|---|---|---|
| `llama-3.1-8b.params` | 8030261248 params | `ASSUMPTION` | G-001, G-008 |
| `llama-3.1-8b.serving.decode_time_share` | 0.9803 fracao do tempo de requisicao | `CONDITIONAL_RESULT` | G-003, G-009 |
| `llama-3.1-8b.serving.energy_per_token` | 6.621 J/token gerado | `CONDITIONAL_RESULT` | G-003, G-004, G-009 |
| `llama-3.1-8b.decode.tokens_per_s` | 174.7 tok/s | `CONDITIONAL_RESULT` | G-003 |
| `llama-3.1-8b.decode.memory_bound_share` | 0.9991 fracao do tempo | `CONDITIONAL_RESULT` | G-003 |
| `llama-3.1-8b.decode.arith_intensity` | 1.059 FLOP/byte | `CONDITIONAL_RESULT` | G-003 |
| `llama-3.1-8b.decode.energy_per_token` | 6.491 J/token | `CONDITIONAL_RESULT` | G-003, G-004 |
...
| `llama-3.1-8b.partition.hardened_share` | 0 fracao do tempo | `ASSUMPTION` | G-001, G-002, G-003, G-006 |
| `llama-3.1-8b.partition.amdahl_ceiling` | 1 x sobre baseline | `ASSUMPTION` | G-001, G-002, G-003, G-006 |
...
| `SRS.llama-3.1-8b` | 0.2757 score SRS | `ASSUMPTION` | G-001, G-002, G-003, G-006, G-008, G-009 |
