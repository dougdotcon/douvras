...
| G-117 | O escore de um modelo de raciocínio híbrido depende do modo tanto quanto do prompt (+18,8 pontos medidos), e **nenhum modelo do corpus foi comparado sob orçamento de tempo igual** — `/think` custa 3,6× por tarefa | Comparação com custo controlado: mesmo tempo de CPU por tarefa para todos os modelos | comparabilidade entre modelos com e sem raciocínio; leitura de qualquer ranking | OPEN |
| G-116 | O `smollm3-3b` foi medido em `/no_think`; o modo padrão dele é `/think`, e o padrão não foi medido por custo de CPU | Mesma suíte em `/think`, com teto de tokens suficiente | leitura do escore do `smollm3-3b` como capacidade do modelo | **PARCIAL** — 16 tarefas (2 por capacidade) em `/think`, pareadas: **12,5 % → 31,2 %**, delta **+18,8 pontos**. O escore publicado de 10,4 % é **piso**, não capacidade. Fechar exige as 96 tarefas em `/think`, ≈ 2,9 h de CPU |
